...
const whitelist = [
  ...
  'http://localhost:3000',
  'https://asn-asmin-widget.vercel.app', // Certifique-se que o nome do app está correto
...
async function sendLeadToNotion(leadData) {
  ...
  try {
    ...
    // --- AJUSTE OS NOMES DAS PROPRIEDADES ABAIXO ---
    const properties = {
      'Nome': { title: [ { text: { content: nome } } ] },
      ...(email && { 'Email': { email: email } }),
      ...(telefone && { 'Telefone': { phone_number: telefone } }),
      ...(origem && { 'Origem': { rich_text: [ { text: { content: origem } } ] } }),
      // ...(origem && { 'Origem': { select: { name: origem } } }), // Para tipo Select
      // 'Data Recebido': { date: { start: new Date().toISOString() } } // Exemplo para Data
    };
...
// --- ROTA DE SAÚDE ---
app.get('/', (req, res) => {
  res.send('VERSÃO 20 DA API. Integração Notion Adicionada. 🚀');
});

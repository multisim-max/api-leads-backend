...
async function sendLeadToNotion(leadData) {
  ...
  try {
    ...
    const properties = {
      ...
      ...(origem && { 'Origem': { select: { name: origem } } }) // Usando tipo Select
    };
...
app.get('/api/logs', checkApiKey, async (req, res) => {
  ...
  } catch (error) { console.error('Erro ao buscar logs:', error); res.status(500).send({ error: 'Erro ao buscar logs.' }); }
});
...
app.post('/api/sources', checkApiKey, async (req, res) => {
  ...
  } catch (error) { console.error('Erro ao criar source:', error); res.status(500).send({ error: 'Erro ao criar fonte.' }); }
});

app.post('/api/mappings', checkApiKey, async (req, res) => {
  const { source_id, mappings } = req.body;
  if (!source_id || !mappings || !Array.isArray(mappings)) { return res.status(400).send({ error: 'Estrutura de dados inválida.' }); }
  const client = await pool.connect();
  try {
    await client.query('BEGIN');
    await client.query('DELETE FROM field_mappings WHERE source_id = $1', [source_id]);
    for (const rule of mappings) {
      if (rule.campo_fonte && rule.tipo_campo_kommo && rule.codigo_campo_kommo) {
        await client.query(
